...
# @profile(stdout=False, filename="jss.prof")
def iterete_over_jobs(current_time):
    ...
    for machine, durations in enumerate(all_durations):
        ...
        if len(durations) <= 1:
            ...
            if current_time != 0:
                ...
                ajpmm = all_jobs_per_machine[machine]
                for job, last_machine in enumerate(output_matrix):
                    if last_machine and last_machine[-1] == machine and job in ajpmm:
                        # Check if the job will continue running in the current_time or not
                        min_index = ajpmm.index(job)
    ...
    for row, job in enumerate(data_with_times):
        if job and job[0][2] == current_time:
            job_0 = job[0]
            output_matrix[row].append(job_0[0])  # job[0][0] is the machine number
            if job_0[1] > 1:
                job_0[1] -= 1  # subtract 1 from the duration of the job
                job_0[2] += 1  # add 1 to the time of the job
            else:
                job.pop(0)
        else:
...
def main(jobs_data):
    ...
    if validate_output_matrix() and validate_output_matrix_2():
        print(f" Makespan={current_time} in {finish_time}")
        # print(f"Output matrix:\n{string_output_matrix()}")
    else:
        print("The output matrix is incorrect")


# Create three test cases for the code
# 1. Create random jobs and save them in a file
# 2. Open the file with the jobs
# 3. Run the main function
# 4. Print the makespan and the time to solve the problem


for file_name in [
    "tests2/instance100_100.txt",
    "tests2/instance200_100.txt",
    "tests2/instance300_100.txt",
    "tests2/instance400_100.txt",
    # "tests2/instance500_100.txt",
]:
...
for JOBS, MACHINES in [
    ...
    with open(f"tests2/instance{JOBS}_{MACHINES}.txt", "w") as file:
        file.write(out2)
